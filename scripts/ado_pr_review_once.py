...
def fetch_pr_by_id(org: str, project: str, pr_id: int, pat_b64: str) -> dict:
    url = f"https://dev.azure.com/{org}/{project}/_apis/git/pullrequests/{pr_id}?api-version=7.1-preview.1"
    return http_get(url, pat_b64)
...
class Finding:
    def __init__(self, severity: str, title: str, details: Optional[str] = None):
...
def check_barrel_export(repo_dir: str, changes: List[Tuple[str, str]], head_ref: str) -> List[Finding]:
    ...
    if missing:
        ...
        findings.append(Finding("WARN", "New modules may be missing from src/index.ts exports", details))
    return findings
...
def check_tests(repo_dir: str, changes: List[Tuple[str, str]], head_ref: str) -> List[Finding]:
    ...
    if no_test_for:
        ...
        if more:
            details += f"\n... and {more} more"
        findings.append(Finding("INFO", "Changed modules without accompanying test changes", details))
    return findings
...
def check_prisma(changes: List[Tuple[str, str]]) -> List[Finding]:
    ...
    if schema_changed and not migrations_changed:
        findings.append(Finding("WARN", "Prisma schema changed without migrations", "Generate and commit a migration if applicable."))
    return findings
...
def review_once(pr: dict, repo_dir: str, remote: str = "origin") -> Tuple[str, str, str]:
    ...
    return title, src + " → " + tgt, header + "\n" + body


def main():
    parser = argparse.ArgumentParser(description="Run a one-off PR review and optionally post to Slack.")
    g = parser.add_mutually_exclusive_group()
    g.add_argument("--pr-id", type=int, help="Specific PR ID to review")
    g.add_argument("--latest", action="store_true", help="Review latest active PR created by me in TARGET_REPO")
    parser.add_argument("--post-slack", action="store_true", help="Post the report to Slack as well")
    args = parser.parse_args()

    org = env("AZDO_ORG", required=True)
    project = env("AZDO_PROJECT", required=True)
    target_repo = env("TARGET_REPO", "dentbird-solutions")
    repo_dir = os.path.expanduser(env("REPO_DIR", f"~/pr-review/{target_repo}"))
    remote = env("GIT_REMOTE", "origin")
    pat = env("AZDO_PAT", required=True)
    webhook = env("SLACK_WEBHOOK", None, required=args.post_slack)
    ...
    if args.pr_id:
        pr = fetch_pr_by_id(org, project, args.pr_id, pat_b64)
        if pr.get("repository", {}).get("name") != target_repo:
            print(f"PR {args.pr_id} is not in target repo {target_repo}", file=sys.stderr)
            sys.exit(2)
    else:
        prs = fetch_my_prs(org, project, creator_id, pat_b64, status="active", top=50)
        # Filter to target repo and sort by creation date desc
        prs = [p for p in prs if (p.get("repository", {}).get("name") == target_repo)]
        prs.sort(key=lambda x: x.get("creationDate") or x.get("createdDate") or "", reverse=True)
        if not prs:
            print("No active PRs found for current user in target repo.")
            return
        pr = prs[0]

    pr_id = pr.get("pullRequestId")
    title, refs, findings_text = review_once(pr, repo_dir, remote=remote)
    url = pr_web_link(org, project, target_repo, pr_id)

    report = f"PR Review — {title}\nRepo: {target_repo} | {refs}\n{url}\n\n{findings_text}"
    print(report)

    if args.post_slack:
        try:
            http_post_json(webhook, {"text": report})
            print(f"[{now_iso()}] Posted to Slack")
        except Exception as e:
            print(f"Failed posting to Slack: {e}", file=sys.stderr)
